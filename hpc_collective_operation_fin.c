...
*/
	...
	if (divisibile != 0){  														// if the number is not divisibile by number of processors i give the number
						   														// divisibile and i will add the extra numbers to the master processor
    ...
    MPI_Reduce(&somma_parziale, &somma, 1, MPI_LONG_LONG, MPI_SUM, 0, MPI_COMM_WORLD); //with this istruction all processor send to master their partial sum
    ...
    end_time_proce=MPI_Wtime();   												// stop the time that rappresent the time for run the program in the master
    																			// (all program) because the master makes the last operation
